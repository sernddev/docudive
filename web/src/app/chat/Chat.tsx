...
import { HEADER_PADDING, MODELS } from "@/lib/constants";
...
export const Chat = ({
  ...
  useEffect(() => {
    ...
    async function initialSessionFetch() {
      ...
      const URL = (modelNumber === MODELS.ModelOne) ? `/v1/api/chat/get-chat-session/${existingChatSessionId}`: `/v2/api/chat/get-chat-session/${existingChatSessionId}`;
  ...
  const [selectedPersona, setSelectedPersona] = useState<Persona | undefined>(updatedPersona);
  ...
  useEffect(() => {
    async function fetchMaxTokens() {
      const URL = modelNumber === 1
      const response = await fetch(
        `/v1/api/chat/max-selected-document-tokens?persona_id=${livePersona.id}`
      );
      if (response.ok) {
  ...
  const onSubmit = async ({
    ...
    if (isNewSession) {
      currChatSessionId = await createChatSession(livePersona?.id || 0, modelNumber);
      eventCycle({key: 'sessionId', value: {modelNumber: modelNumber, sessionId: currChatSessionId}} );
    } else {
    ...
    eventCycle({key: 'isStreaming', value: true})
    setIsStreaming(true);
    ...
    eventCycle({key: 'isStreaming', value: false});
    ...
    if (isNewSession) {
      ...
      await nameChatSession(currChatSessionId, currMessage, modelNumber);
  ...
  return (
    <div className="flex w-full overflow-x-hidden" ref={masterFlexboxRef}>
      ...
      {documentSidebarInitialWidth !== undefined ? (
        <>
          <div
            className={`w-full sm:relative relative h-screen ${
              retrievalDisabled ? "pb-[111px]" : "pb-[90px]"
            }`}
          >
            
            <div className={`chat-window w-full h-full ${HEADER_PADDING} flex flex-col overflow-y-auto overflow-x-hidden relative`}
