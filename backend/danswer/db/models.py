...
class AuditLog(Base):
    ...
    operation: Mapped[str] = mapped_column(String, nullable=True) # INSERT, UPDATE, DELETE
    audit_stage:Mapped[str] = mapped_column(String, nullable=True)
    record_id:Mapped[str] = mapped_column(Text, primary_key=False)
    timestamp : Mapped[datetime.datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
    changes =  mapped_column(Text, default=None)
